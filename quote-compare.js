export async function onRequestPost(context) {
  ...
  const { quoteAText, quoteBText, notes } = body;

  if (!quoteAText || !quoteBText) {
    return new Response(JSON.stringify({ error: 'quoteAText and quoteBText are required' }), { status: 400 });
  }

  if (quoteAText.length > 500000 || quoteBText.length > 500000) {
  ...
  try {
    ...
    if (webhookSecret) headers['x-quotescan-secret'] = webhookSecret;

    const response = await fetch(webhookUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ quoteAText, quoteBText, notes: notes || '' }),
